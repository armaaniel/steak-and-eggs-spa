...
const TraceOverviewTable = ({traceData, recordsPerPage, error}: {traceData: TraceSummary[], recordsPerPage: number, error:ApolloError | undefined}) => {
	...
	<table className='dc-table'>
		<thead>
		<tr>
			<th className='dc-row-heading' onClick={() => setSorted(false)}>Route</th>
			<th className={sorted && sortField === 'p99' ? (direction === 'asc' ? 'dc-row-heading-asc' : 
			'dc-row-heading-desc') : 'dc-row-heading'} onClick={() => handleSort('p99')}>p99</th>
			<th className={sorted && sortField === 'totalRequests' ? (direction === 'asc' ? 'dc-row-heading-asc' : 
